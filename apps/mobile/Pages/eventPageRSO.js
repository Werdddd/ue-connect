...
export default function EventPageRSO() {
  ...
  const [locations] = useState([
    ...
    {
      label: "EE Laboratory Rooms, EN Building",
      value: "EE Laboratory Rooms, EN Building",
    },
    {
      label: "EN Briefing Room, EN Building",
      value: "EN Briefing Room, EN Building",
    },
    { label: "MPH 3, LCT Building", value: "MPH 3, LCT Building" },
    {
      label: "Conference Hall, TYK Building",
      value: "Conference Hall, TYK Building",
    },
  ]);
  ...
  const handleAddEvent = async () => {
    ...
    const conflictingEvents = events.filter((event) => {
      ...
      try {
        ...
        const hasOverlap =
          incoming.start < existing.end && incoming.end > existing.start;
        return hasOverlap;
      } catch (err) {
    ...
    if (conflictingEvents.length > 0) {
      const conflictDetails = conflictingEvents
        .map(
          (event) => `  • "${event.title}" at ${event.time} (${event.status})`
        )
        .join("\n");
    ...
    if (newDate && newTime && newLocation) {
      ...
      const conflictingEvents = events.filter((event) => {
        if (!(event.status === "Applied" || event.status === "Approved"))
          return false;
        if (
          normalizeLocation(event.location) !== normalizeLocation(newLocation)
        )
          return false;
      ...
      if (conflictingEvents.length > 0) {
        const details = conflictingEvents
                ...
                <View style={styles.modalContent}>
                      ...
                      {selectedProposal && (
                        <View style={styles.proposalPreview}>
                          ...
                          <View style={styles.proposalDetails}>
                            ...
                            <TouchableOpacity
                              onPress={() =>
                                Linking.openURL(selectedProposal.base64)
                              }
                            >
                              <Text style={styles.proposalViewText}>
                                View PDF
                              </Text>
                            </TouchableOpacity>
                          </View>
                            ...
                            return (
                              <TouchableOpacity
                                ...
                                onPress={() => {
                                  if (isChecked) {
                                    setSelectedOrgs(
                                      selectedOrgs.filter((o) => o !== org.id)
                                    );
                                  } else {
                  ...
                  <View style={styles.modalButtons}>
                    ...
                    <TouchableOpacity
                      style={[
                        styles.addButton,
                        isAddingEvent && styles.disabledButton,
                      ]}
