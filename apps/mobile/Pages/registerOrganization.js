...
export default function RegisterOrganization() {
  ...
  const courses = [
    ...
    {
      label: "College of Fine Arts, Architecture and Design",
      value: "label-cfad",
      isLabel: true,
    },
    { label: "BMA", value: "BMA" },
    ...
    try {
      ...
      if (result.type === "success" || !result.canceled) {
        ...
        if (file.size > firestoreSafeLimit) {
          Alert.alert(
            "⚠️ File Too Large",
            `The selected file "${file.name}" is ${(
              file.size /
              (1024 * 1024)
            ).toFixed(
              2
            )} MB, which is too large to store.\n\nPlease upload a smaller file (<750KB).`,
            [
              {
                text: "Re-upload",
  ...
  const validateForm = () => {
    ...
    if (selectedCourses.length === 0) {
      Alert.alert(
        "Error",
        "Please select at least one course that can join your organization"
      );
      return false;
    }
    ...
    try {
      ...
      if (!result.canceled) {
        ...
        if (file.fileSize && file.fileSize > maxSize) {
          Alert.alert(
            "⚠️ Logo Too Large",
            `The selected image "${file.fileName || "logo"}" is ${(
              file.fileSize /
              (1024 * 1024)
            ).toFixed(
              2
            )} MB, which exceeds the 5MB limit.\n\nPlease upload a smaller image.`,
            [
              {
                text: "Re-upload",
                onPress: () => pickLogo(),
          ...
          <View style={styles.section}>
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput
                ...
                onChangeText={(text) =>
                  handleInputChange("presidentName", text)
                }
              />
            </View>
